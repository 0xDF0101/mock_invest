...
│   ├── market/      # MarketHoursService (장 마감 카운트다운)
│   └── ranking/     # RankEntry, RankingService (수익률 랭킹)
├── event/           # TradeCompletedEvent
├── infrastructure/
│   └── stockprice/  # StockPriceProvider 인터페이스 + YahooFinanceProvider
└── web/
    ├── controller/  # AuthController, DashboardController, StockController, TradeController, RankingController
    └── dto/         # RegisterRequest, TradeRequest, PortfolioDto, HoldingDto

src/main/resources/
├── db/migration/    # V1(users) V2(stocks) V3(holdings) V4(trades)
├── db/seed/         # V5(dev_admin) V6(korean_stock_names) — dev 프로파일 전용
└── templates/       # auth/, dashboard/, stock/, trade/, ranking/, fragments/
```
...
| `GET /ranking` | 수익률 랭킹 (Top 10 + 내 순위) |
...
새 기능 구현 시 핵심 도메인 로직에 대한 기초 단위 테스트를 함께 작성한다.
- 도구: JUnit 5 + Mockito (spring-boot-starter-test 포함)
- 범위: Service 레이어 단위 테스트 중심 (Repository는 Mock)
